...
<script setup>
	...
	function getList(){
		...
		myApi('imgSort',searchForm.value).then((res)=>{
	...
	function add(){
		refAddForm.value.validate((valid)=>{
			if(valid){
				myApi('addSort',addForm.value,'post').then((res)=>{
	...
	function editSub(){
		refEditForm.value.validate((valid)=>{
			if(valid){
				myApi('upSort',editForm.value,'post').then((res)=>{
	...
	function del(scope){
		myApi('delSort',{id:scope.row.id},'post').then((res)=>{
			ElMessage({message:res.msg,type:'success'})
			getList()
		})
	}
