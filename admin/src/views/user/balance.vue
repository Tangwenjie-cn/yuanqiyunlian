		...
		<el-form :inline="true" :model="searchForm">
			<el-form-item class="page-search-form-item page-search-select">
				<el-select v-model="searchForm.type" clearable placeholder="类型">
					<el-option :value="1" label="推广奖励"></el-option>
					<el-option :value="2" label="高级推广奖励"></el-option>
                    <el-option :value="3" label="提现"></el-option>
                    <el-option :value="4" label="余额购买"></el-option>
				</el-select>
			</el-form-item>
	...
	<div class="page-table">
		<el-table :data="list" stripe border table-layout="fixed" style="width: 100%">
			...
			<el-table-column align="center" label="金额" width="120" >
                <template #default="scope">
                    <el-text v-if="scope.row.mode==1" type="primary">+{{scope.row.money}}</el-text>
					<el-text v-else type="danger">-{{scope.row.money}}</el-text>
				</template>
			</el-table-column>
			<el-table-column align="center" label="类型" width="120">
				<template #default="scope">
					<span v-if="scope.row.type==1">推广奖励</span>
					<span v-else-if="scope.row.type==2">高级推广奖励</span>
                    <span v-else-if="scope.row.type==3">提现</span>
                    <span v-else-if="scope.row.type==4">余额购买</span>
				</template>
			</el-table-column>
...
<script setup>
	...
	function getList(){
		...
		myApi('BalanceList',searchForm.value).then((res)=>{
